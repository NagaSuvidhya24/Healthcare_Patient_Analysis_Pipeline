...
1️⃣	Setup Catalog and Schema	01_setup_catalog_and_schema	Create catalog, schemas & tables
2️⃣	Bronze Layer Ingestion	02_ingest_bronze	Load raw data into Bronze tables
3️⃣	Silver Transformations	03_transform_silver	Cleaning, validations, deduplication
4️⃣	Gold Layer Aggregations	04_aggregate_gold	Build analytics-ready datasets
...
2️⃣ Create external location connected to AWS S3
3️⃣ Create Workflow → Add all tasks → Set schedule
4️⃣ Validate output in Bronze → Silver → Gold tables
5️⃣ View dashboards for final analytics
